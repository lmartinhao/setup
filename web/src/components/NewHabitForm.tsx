...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if(weekDays.includes(weekDay)) {
      const weekDaysWithouRemovedOne = weekDays.filter(day => day !== weekDay);
      setWeekDays(weekDaysWithouRemovedOne);
    } else {
      ...
      <div className='flex flex-col gap-2 mt-3'>
        {availableWeekDays.map((weekDay, i) => {
          return (
            ...
            >
              <div className='w-8 h-8 rounded-lg flex items-center justify-center bg-zinc-900 border-2 border-zinc-800 group-data-[state=checked]:bg-green-500 group-data-[state=checked]:border-green-500 transition-colors group-focus:ring-2 group-focus:ring-violet-600 group-focus:ring-offset-2 group-focus:ring-offset-zinc-900'>
                <Checkbox.Indicator>
